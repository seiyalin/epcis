...
<head>
	...
	function doSelectAll(){
  	...
  	//新增
  	function doAdd(){
  		document.forms[0].action = "${basePath}syfw/role_addUI.action";
  		document.forms[0].submit();
  	}
  	//编辑
  	function doEdit(id){
  		document.forms[0].action = "${basePath}syfw/role_editUI.action?role.roleId=" + id;
  		document.forms[0].submit();
  	}
  	//删除
  	function doDelete(id){
  		document.forms[0].action = "${basePath}syfw/role_delete.action?role.roleId=" + id;
  		document.forms[0].submit();
  	}
  	//批量删除
  	function doDeleteAll(){
  		document.forms[0].action = "${basePath}syfw/role_deleteSelected.action";
  		document.forms[0].submit();
  	}
  	var list_url = "${basePath}syfw/role_listUI.action";
	//搜索
  	function doSearch(){
  		//重置页号
  		$("#pageNo").val(1);
