...
public class CommonUtils {
	...
	public static String changeUnit(int size){
		if(size  < 1024){
			...
			return new DecimalFormat("0.00").format(size/1024.0) + "M";
		}else{
			return new DecimalFormat("0.00").format(size/1024.0/1024.0) + "G";
		}
	}
	
	public static String builtString(Integer id,Integer length){
		String result = id.toString();
		int idLength = result.length();
		if(idLength<length){
			for(int i=0;i<length-idLength;i++){
				result="0"+result;
			}
		}
		return getRandomString(2)+result;
	}
