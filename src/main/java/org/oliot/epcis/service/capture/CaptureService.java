...
public class CaptureService implements CoreCaptureService {
	...
	public void capture(ObjectEventType event) {
		...
		System.out.println("调用了capture(ObjectEventType event)");
	...
	public void capture(EPCISDocumentType epcisDocument) {
         ...
         System.out.println("调用与EventCapture相关联的capture(EPCISDocumentType epcisDocument)方法");
		...
		//测试
		System.out.println("屈岳测试：eventList.size() = "+eventList.size());
		/*
		...
		for (int i = 0; i < eventList.size(); i++) {
			...
			System.out.println("测试JAXBElement中的QName ----开始");
			
			QName name = eventElement.getName();
			...
			System.out.println("测试JAXBElement中的QName ----结束");
