	...
	public ObjectEventType convert(DBObject dbObject) {
		...
		try {
			...
			if (dbObject.get("extension") != null) {
				...
				}if(extObject.get("otherAttributes") != null){//此处屈岳新增，为了获取otherAttributes(Map中)所有键值对信息，并封装到oeet对象当中，供jsp页面前台展示
//					BasicDBList
					
//					BasicDBList basicDBList = (BasicDBList)extObject.get("otherAttributes");
					Map<QName, String> otherAttributes = oeet.getOtherAttributes();
					BasicDBObject otherAttributeObject = (BasicDBObject) extObject
							.get("otherAttributes");
...
//					oeet.setOtherAttributes(otherAttributes);
				}//以上屈岳修改
				// extension2
