...
public class Configuration implements ServletContextListener {
	...
	public static String backendForMySql;//Configuration 中配置支持MySQL
   ...
   //当启动web应用时tomcat调用该方法
	...
	private void setBasicConfiguration(ServletContext context) {
		...
		try {
			...
			// 设置后台是否支持MYSQL数据库，用来保存水产养殖用户注册信息
			String backendForMySql = json.getString("backendForMySql");
			if (backendForMySql == null) {
				Configuration.logger
				.error("backendForMySql字段为空，暂不支持MYSQL，需手动配置Configuration.json来开启MYSQL数据库服务！！");
			} else {
				Configuration.backendForMySql = backendForMySql;
				Configuration.logger.info("Backend - " + Configuration.backend);
			}
	...
	private void loadExistingSubscription() {
	    ...
	    if(Configuration.backendForMySql.equals("MySQL")){
	    	//开启MySql
	    }
	}
}
